...
%Hough Transform
theta_angle = 15; % limit Thera (vertical line) degree
[Hc,Tc,Rc] = hough(I_canny,'theta',-theta_angle:0.01:theta_angle);
[Hs,Ts,Rs] = hough(I_canny,'theta',-theta_angle:0.01:theta_angle);

%Find 4 peaks in the Hough transform matrix, H,
P_sobel = houghpeaks(Hs, 12,'threshold',ceil(0.5*max(Hs(:))));
P_canny = houghpeaks(Hc, 12,'threshold',ceil(0.5*max(Hc(:))));
