...
I_rgb = imread('Image_acq/RES/C910_M1/T1_L1.jpg'); %Source IMG
I_hsv = rgb2hsv(I_rgb);
...
bg_disk = imopen(I_hsv3,strel('disk',15));
bg_disk_blur = imgaussfilt(bg_disk,3); % Smooth version 2sigma
...
Canny_threshold = 0.2;
I_sobel = edge(I_tmp_blur,'sobel',Sobel_threshold);
I_canny = edge(I_tmp_blur,'canny',Canny_threshold);
