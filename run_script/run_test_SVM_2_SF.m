...
run_SVM_2;
disp('---------------------------------------------------');
disp('    2-Features(Shape + Texture FRR) SVM Model');
disp('---------------------------------------------------');
% test vector size
...
X(nRowPos+nRowNeg,nColShape+nCol_T_FRR) = zeros; %test feature row vector
X(1:nRowPos,:) = [test_shF_Pos test_teF_FRR_Pos];
X((nRowPos+1):end,:) = [test_shF_Neg test_teF_FRR_Neg];
    
[label_L,~] = predict(SVM_sTfrr_linear,X);
[label_G,~] = predict(SVM_sTfrr_gaussian,X);
[label_P,~] = predict(SVM_sTfrr_poly,X);
