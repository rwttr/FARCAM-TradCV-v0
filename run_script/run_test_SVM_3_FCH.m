...
run_SVM_3;
disp('---------------------------------------------------');
disp('    3-Features(FRR + CMT + HOG) SVM Model');
disp('---------------------------------------------------');
% test vector size
...
%test feature row vector
X(nRowPos+nRowNeg,nColFRR+ nColCMT+ nColHOG) = zeros; 
X(1:nRowPos,:) = [test_teF_FRR_Pos test_teF_CMT_Pos test_teF_HOG_Pos];
X((nRowPos+1):end,:) = [test_teF_FRR_Neg test_teF_CMT_Neg test_teF_HOG_Neg];
    
[label_L,~] = predict(SVM_fCH_linear,X);
[label_G,~] = predict(SVM_fCH_gaussian,X);
[label_P,~] = predict(SVM_fCH_poly,X);
