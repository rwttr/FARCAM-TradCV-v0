...
run_extTestFeature;
run_SVM_1;
disp('Single Feature(Texture:HOG) SVM Model');
disp('---------------------------------------------------');
% test vector size
...
X(nRowPos+nRowNeg,nCol_T_HOG) = zeros; %test feature row vector
X(1:nRowPos,:) = test_teF_HOG_Pos;
X((nRowPos+1):end,:) = test_teF_HOG_Neg;
    
[label_L,~] = predict(SVM_tHog_linear,X);
[label_G,~] = predict(SVM_tHog_gauss,X);
[label_P,~] = predict(SVM_tHog_poly,X);
