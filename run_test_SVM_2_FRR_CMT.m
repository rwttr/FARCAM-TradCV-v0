...
run_SVM_2;
disp('--------------------------------------------------------');
disp('    2-Features(Texture FRR + Co-oc Matrix) SVM Model');
disp('--------------------------------------------------------');
% test vector size
...
X(nRowPos+nRowNeg,nCol_T_FRR+nCol_T_CMT) = zeros; %test feature row vector
X(1:nRowPos,:) = [test_teF_FRR_Pos test_teF_CMT_Pos];
X((nRowPos+1):end,:) = [test_teF_FRR_Neg test_teF_CMT_Neg];
    
[label_L,~] = predict(SVM_tFrrCmt_linear,X);
[label_G,~] = predict(SVM_tFrrCmt_gaussian,X);
[label_P,~] = predict(SVM_tFrrCmt_poly,X);
