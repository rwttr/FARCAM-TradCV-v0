...
run_SVM_3;
disp('---------------------------------------------------');
disp('    3-Features(Shape + CMT + HOG) SVM Model');
disp('---------------------------------------------------');
% test vector size
...
X(nRowPos+nRowNeg,nColShape+nColCMT +nColHOG) = zeros; %test feature row vector
X(1:nRowPos,:) = [test_shF_Pos test_teF_CMT_Pos test_teF_HOG_Pos];
X((nRowPos+1):end,:) = [test_shF_Neg test_teF_CMT_Neg test_teF_HOG_Neg];
    
[label_L,~] = predict(SVM_sCH_linear,X);
[label_G,~] = predict(SVM_sCH_gaussian,X);
[label_P,~] = predict(SVM_sCH_poly,X);
